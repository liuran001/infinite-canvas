...
- 外部服务请求统一放在 `web/src/services/api/`。项目同时支持「本地模式」和「服务器模式」：本地模式由浏览器直连用户自己的接口，服务器模式经由 `server/` 后端代理并持久化。
- 新增涉及数据读写或 AI 调用的功能时，必须同时考虑两种模式；服务端逻辑一律用 `isServerMode()` 判断后再走，**不允许改变本地模式的既有行为**。
- 服务器模式的状态放在 `web/src/stores/use-server-store.ts`，后端接口客户端放在 `web/src/services/api/server.ts`，两者是双模式的公共契约，改动前先确认调用方。
...
- 本地模式下画布项目和「我的素材」保存在浏览器本地，仅有 WebDAV 可选同步；服务器模式下才保存在服务端并支持多设备同步。写文档时要区分两种模式，不要笼统地说「已支持云同步」。
- 本地模式的 AI API Key 存在浏览器本地并由前端直接请求 OpenAI 兼容接口；服务器模式的渠道密钥只存在服务端，不下发给前端。涉及安全说明时要写清楚是哪种模式。
