...
/** 云空间变化。与余额一样发绝对值：断线重连必然丢事件，增量会让界面上的数字永久偏掉。 */
export type TeamStorageEvent = { type: "team.storage"; teamId: string; storageUsed: number; storageQuota: number };
 ...
 * 两个数一起发而不是只发上限：前端那一条进度条要两个数才画得出来，只给上限的话它还得自己再打一次请求，
 * 而那次请求读到的用量与这次事件之间又隔着一个窗口。
 */
export function publishTeamStorage(teamId: string, storageUsed: number, storageQuota: number) {
    bus.emit(channel(teamId), { type: "team.storage", teamId, storageUsed, storageQuota } satisfies TeamStorageEvent);
}
