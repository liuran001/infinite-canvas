...
import { canTeamAction, requireTeamRole } from "./team-access";
...
export async function getTeam(userId: string, teamId: string) {
    const { team, role } = await requireTeamRole(userId, teamId, ["owner", "admin", "member", "viewer"]);
    return teamView(team, role);
}

export async function updateTeam(teamId: string, actorId: string, input: TeamInput) {
    const { team, role } = await requireTeamRole(actorId, teamId, ["owner", "admin"], { write: true });
    const next = {
...
export async function listMembers(userId: string, teamId: string) {
    await requireTeamRole(userId, teamId, ["owner", "admin", "member", "viewer"]);
...
async function activeMemberOrFail(teamId: string, userId: string) {
...
export async function updateMemberRole(teamId: string, actorId: string, targetId: string, role: TeamRole) {
    const actor = await requireTeamRole(actorId, teamId, ["owner", "admin"], {
        write: true,
    });
    const target = await activeMemberOrFail(teamId, targetId);
    ...
    // admin 之间互不干涉，且只有 owner 能造出新的 admin。
    if (actor.role === "admin" && target.role === "admin") throw fail("admin 不能操作同级 admin", 403, "TEAM_FORBIDDEN");
...
export async function removeMember(teamId: string, actorId: string, targetId: string) {
    ...
    if (actor.role === "admin" && target.role === "admin") throw fail("admin 不能移除同级 admin", 403, "TEAM_FORBIDDEN");
    await repo(TeamMember).delete({ teamId, userId: targetId });
}

export async function leaveTeam(teamId: string, userId: string) {
    const { role } = await requireTeamRole(userId, teamId, ["owner", "admin", "member", "viewer"], { write: true });
...
export async function transferOwner(teamId: string, actorId: string, targetId: string) {
    await requireTeamRole(actorId, teamId, ["owner"], { write: true });
