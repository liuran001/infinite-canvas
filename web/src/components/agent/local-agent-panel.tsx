...
import { History, MessageSquare, PanelRightClose, PlugZap, Plus, Sparkles, Terminal } from "lucide-react";
...
export function LocalAgentPanel({ embedded, headless, autoConnect, forceLocal = false, shareRestricted = false }: { embedded?: boolean; headless?: boolean; autoConnect?: boolean; forceLocal?: boolean; shareRestricted?: boolean }) {
    ...
    useEffect(() => {
        ...
        setAgentState({ enabled: false, connected: false, silentConnect: false, activity: rt("offline"), connectError: "" });
        return () => {
            manualConnectionRef.current = false;
            connectedRef.current = false;
            const state = useAgentStore.getState();
            if (state.enabled || state.connected) setAgentState({ enabled: false, connected: false, silentConnect: false, activity: rt("offline") });
        };
    ...
    }, [applyConversationState, applyWorkspaceChange, clientReady, enabled, endpoint, loadSkills, loadThreads, message, restricted, setAgentState, token]);
    ...
    const sendPrompt = async () => {
        ...
        let messageReferences: AgentCanvasReference[] = [];
        try {
            ...
            messageReferences = await Promise.all(canvasReferences.map(async ({ nodeId, label, title, kind, previewUrl, text }) => {
            ...
            <AgentPanelTabs
                ...
                leading={
                    <div className="flex items-center gap-1">
                        <AgentModeSwitch theme={theme} forceLocal={forceLocal} />
