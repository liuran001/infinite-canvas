...
export function AgentChatMessage({ item, theme, onRejectTool, onApproveTool }: { item: AgentChatMessageItem; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onRejectTool?: (id: string) => void; onApproveTool?: (id: string) => void }) {
    ...
    return (
        <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
            ...
            >
                {isUser ? (
                    item.body || <AgentUserMessageContent text={item.text} references={item.canvasReferences || []} skill={item.skill} theme={theme} />
                ) : (
...
export function AgentPendingToolCard({ summary, detail, theme, title, deciding, onReject, onApprove }: { summary: string; detail?: unknown; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; title?: string; deciding?: boolean; onReject?: () => void; onApprove?: () => void }) {
    const { t } = useTranslation();
            ...
            <details className="group">
                <summary className={`list-none ${view ? "cursor-pointer" : "cursor-default"}`} onClick={(event) => { if (!view) event.preventDefault(); }}>
                    <div className="flex min-w-0 items-center gap-2 text-sm font-medium leading-5">
                        ...
                        <span className="min-w-0 flex-1">{title || t("agent.message.awaitingConfirmation")}</span>
    ...
    return (
        <details className="group min-w-0 text-left">
            ...
            <div className="ml-6 mt-1 space-y-2">
                {items.map((item) => <AgentToolCard key={item.id} title={item.title || tr("toolCall")} text={item.text} detail={item.detail} theme={theme} />)}
            </div>
        </details>
    );
}
