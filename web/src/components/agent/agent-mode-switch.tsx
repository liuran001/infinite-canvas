...
const MODES: Array<{ value: AgentPanelMode; label: string; description: string; icon: typeof Cloud }> = [
    { value: "cloud", label: "agent.cloud.mode.cloud", description: "agent.cloud.mode.cloudDescription", icon: Cloud },
    { value: "local", label: "agent.cloud.mode.local", description: "agent.cloud.mode.localDescription", icon: Laptop },
];
...
export function AgentModeSwitch({ theme, forceLocal = false }: { theme: (typeof canvasThemes)[keyof typeof canvasThemes]; forceLocal?: boolean }) {
    ...
    return (
                    ...
                    label: (
                        <div className="flex min-w-[190px] items-center gap-2 py-0.5">
                            ...
                            <div className="min-w-0 flex-1">
                                <div className="text-sm leading-5">{t(item.label)}</div>
                                <div className="text-[11px] leading-4 opacity-60">{t(item.description)}</div>
                            </div>
        ...
        >
            <button type="button" className="flex h-8 items-center gap-1.5 rounded-md px-1 transition hover:bg-black/5 dark:hover:bg-white/10" style={{ color: theme.node.text }} aria-label={t("agent.cloud.mode.switch")}>
                <Icon className="size-4 shrink-0" />
                <span className="text-base font-semibold leading-5">{t(current.label)}</span>
