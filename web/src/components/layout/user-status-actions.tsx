...
export function UserStatusActions({ showConfig = true, variant = "default", onOpenShortcuts, onOpenPlugins }: UserStatusActionsProps) {
    ...
    useEffect(() => {
        ...
        if (bound) message.success(t("topNav.user.linuxDoBound"));
    ...
    const menuItems: MenuProps["items"] = [
        {
            ...
            label: (
                <div className="w-56 py-1">
                    ...
                    <div className="mt-0.5 text-xs">{t("topNav.user.credits", { credits: user?.credits ?? 0 })}</div>
                    {storage ? (
                        <>
                            ...
                            <div className={`text-xs ${storageNearlyFull ? "font-medium text-red-500" : ""}`}>
                                {t("topNav.user.storage", { used: formatBytes(storage.used), quota: formatBytes(storage.quota) })}
                            </div>
                            {storageNearlyFull ? <div className="mt-1 text-xs text-red-500">{t("topNav.user.storageNearlyFull")}</div> : null}
                        </>
                    ) : null}
                </div>
            ),
        ...
        { type: "divider" },
        { key: "account", icon: <Settings2 className="size-4" />, label: t("topNav.user.accountSettings"), onClick: () => setAccountOpen(true) },
        ...(user?.role === "admin" ? [{ key: "admin", icon: <Shield className="size-4" />, label: t("topNav.user.admin"), onClick: () => navigate("/admin") }] : []),
        { type: "divider" },
        { key: "logout", icon: <LogOut className="size-4" />, label: t("topNav.user.logout"), onClick: () => useServerStore.getState().clearSession() },
    ];
            ...
            {user ? (
                <Dropdown trigger={["click"]} placement="bottomRight" menu={{ items: menuItems }}>
                    <button
                        ...
                        style={iconStyle}
                        aria-label={t("topNav.user.accountLabel")}
