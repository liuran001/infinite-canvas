...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    }, [data.id, data.title, onTitleChange, readOnly, t, titleDraft]);
...
function ImageContent({
    ...
    return (
        <BatchFrame batchCount={batchCount} batchExpanded={batchExpanded} onToggleBatch={onToggleBatch}>
                ...
                ? images
                      ...
                      .map((image, index) => (
                          <ExpandedImageCard
                              ...
                              index={index}
                              readOnly={readOnly}
                              onSetPrimary={onSetBatchPrimary ? () => onSetBatchPrimary(image.id) : undefined}
                              onDuplicate={onDuplicateBatchImage ? () => onDuplicateBatchImage(image.id) : undefined}
                              onDownload={onDownloadBatchImage ? () => onDownloadBatchImage(image.id) : undefined}
                              onRetry={onRetryBatchImage ? () => onRetryBatchImage(image.id) : undefined}
                              onDelete={onDeleteBatchImage ? () => onDeleteBatchImage(image.id) : undefined}
                          />
                      ))
                : null}
            ...
            {!readOnly && primaryImage?.status === "error" && onRetryBatchImage && onDeleteBatchImage ? <BatchImageFailureActions placement="left" onRetry={() => onRetryBatchImage(primaryImage.id)} onDelete={() => onDeleteBatchImage(primaryImage.id)} /> : null}
            {primaryImage?.content && onDownloadBatchImage ? (
...
function ExpandedImageCard({ node, image, index, readOnly, onSetPrimary, onDuplicate, onDownload, onRetry, onDelete }: { node: CanvasNodeData; image: CanvasNodeImage; index: number; readOnly: boolean; onSetPrimary?: () => void; onDuplicate?: () => void; onDownload?: () => void; onRetry?: () => void; onDelete?: () => void }) {
    const theme = canvasThemes[useThemeStore((state) => state.theme)];
    const { t } = useTranslation();
    ...
    return (
        ...
        >
            ...
            {image.content && (onDownload || (!readOnly && (onDuplicate || onSetPrimary))) ? (
                <div className="absolute inset-x-2 top-2 flex items-center gap-1">
                    {onDownload ? <button type="button" className="flex h-8 min-w-0 flex-1 items-center justify-center gap-1 rounded-lg border px-1.5 text-[10px] font-medium shadow-[0_6px_18px_rgba(15,23,42,.16)] backdrop-blur-md transition hover:scale-[1.02]" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.toolbar.activeText }} title={t("common.download")} onClick={(event) => (event.stopPropagation(), onDownload())}>
                    ...
                    {!readOnly && onSetPrimary ? <button type="button" className="flex h-8 min-w-0 flex-1 items-center justify-center gap-1 rounded-lg border px-1.5 text-[10px] font-medium shadow-[0_6px_18px_rgba(15,23,42,.16)] backdrop-blur-md transition hover:scale-[1.02]" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.toolbar.activeText }} title={t("canvas.node.setPrimary")} onClick={(event) => (event.stopPropagation(), onSetPrimary())}>
                        <Star className="size-3 shrink-0" style={{ color: selectionBlue }} />
                        <span className="truncate">{t("canvas.node.setPrimary")}</span>
                    </button> : null}
                </div>
            ) : null}
            {!readOnly && image.status === "error" && onRetry && onDelete ? <BatchImageFailureActions placement="right" onRetry={onRetry} onDelete={onDelete} /> : null}
        </div>
    );
}
