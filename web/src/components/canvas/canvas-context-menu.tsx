...
export function CanvasNodeContextMenu({ menu, onClose, onDuplicate, onDelete, readOnly = false, node, onInfo, onDownload, onViewImage }: { menu: ContextMenuState; onClose: () => void; onDuplicate: () => void; onDelete: () => void; readOnly?: boolean; node?: CanvasNodeData; onInfo?: () => void; onDownload?: () => void; onViewImage?: () => void }) {
    ...
    return (
        ...
        >
            {readOnly && menu.type === "node" ? <MenuButton icon={<Info className="size-4" />} label={t("common.details")} onClick={onInfo} /> : null}
            {readOnly && menu.type === "node" && node?.metadata?.content && ["image", "video", "audio"].includes(node.type) ? <MenuButton icon={<Download className="size-4" />} label={t("common.download")} onClick={onDownload} /> : null}
            {readOnly && menu.type === "node" && node?.type === "image" && node.metadata?.content ? <MenuButton icon={<Plus className="size-4" />} label={t("common.view")} onClick={onViewImage} /> : null}
            {!readOnly && menu.type === "node" ? <MenuButton icon={<Plus className="size-4" />} label={t("canvas.controls.duplicate")} onClick={onDuplicate} /> : null}
            {!readOnly ? <MenuButton icon={<Trash2 className="size-4" />} label={t("canvas.controls.delete")} onClick={onDelete} danger /> : null}
        </div>
    );
}
