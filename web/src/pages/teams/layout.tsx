...
const tabs = [
    { key: "", labelKey: "teams.tabs.overview" },
    { key: "members", labelKey: "teams.tabs.members" },
    { key: "invites", labelKey: "teams.tabs.invites" },
    { key: "logs", labelKey: "teams.tabs.logs" },
] as const;
...
export default function TeamLayout() {
    ...
    if (isPending) return <main className="flex h-full items-center justify-center bg-background text-sm text-stone-500">{t("teams.common.loading")}</main>;
    if (error || !data) return <main className="flex h-full items-center justify-center bg-background text-sm text-red-500">{error instanceof Error ? error.message : t("teams.layout.loadFailed")}</main>;
                ...
                <header className="border-b border-stone-200 pb-5 dark:border-stone-800">
                    ...
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                        <div className="min-w-0">
                            ...
                            <p className="mt-1 text-sm text-stone-500">{data.description || t("teams.common.noDescription")}</p>
                        </div>
                        <div className="text-right">
                            <div className="text-xs text-stone-500">{t("teams.common.credits")}</div>
                            <div className="text-2xl font-semibold tabular-nums" data-testid="team-credits">
                                {credits}
                            </div>
                            {/* 断开之后余额就停在最后收到的那个数上，不说明白的话用户会一直把它当成真值。 */}
                            {realtimeStatus === "failed" ? (
                                <div className="mt-1 text-xs text-red-500">{realtimeError || t("teams.layout.realtimeDisconnected")}</div>
                            ) : (
                                <div className="mt-1 text-xs text-stone-400">{t(realtimeStatus === "polling" ? "teams.layout.realtimePolling" : realtimeStatus === "ready" ? "teams.layout.realtimeReady" : "teams.layout.realtimeConnecting")}</div>
                            )}
                        </div>
                    </div>
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                        <Tag className="m-0">{t("teams.layout.myRole", { role: t(`teams.roles.${role}`) })}</Tag>
                        {data.status !== "active" ? <Tag color="warning">{t("teams.layout.disabled")}</Tag> : null}
                    </div>
                    <nav className="mt-4 flex items-center gap-4 text-sm" aria-label={t("teams.layout.navLabel")}>
                        {tabs.map((tab) => (
                            <Link
