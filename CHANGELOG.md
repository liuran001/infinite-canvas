...
+ [新增] 画布右侧 Agent 面板新增「系统模型」模式：由服务端模型直接读写当前画布，关掉面板、断网或换设备都不会中断执行，重新进入会自动补齐进度并把画布刷新到最新；支持新建、切换与删除会话，运行中可随时中止，并明确展示「按轮扣算力点」的计费口径。
+ [新增] 画布右侧「系统模型」Agent 支持用户自己选模型：可选范围是服务端已配置的文本模型并标出每轮算力点，计费按所选模型走；每个会话记住自己在用的模型，另有独立的「Agent 默认模型」偏好决定新建会话用哪个，跟随账号云端同步，没设置过时沿用管理员配置的全站默认，选中的模型被管理员下线后自动回落到默认。
