...
+ [新增] 恢复服务器后端，支持账号登录、Linux.do 授权登录与算力点体系。
+ [新增] 新增服务器模式，画布项目、素材与图片可保存在服务器，支持多设备同步。
+ [新增] 生图、视频、音频改由服务端任务队列执行，客户端断网、刷新或更换设备都能恢复进度，不会重复生成或重复扣费。
+ [新增] 恢复管理后台，可管理用户、算力点流水、模型渠道、系统设置、提示词与素材。
+ [新增] 服务端文件存储支持本地磁盘与 S3 兼容对象存储两种驱动。
+ [新增] 生图、文本、视频、音频四类功能入口默认按当前可用模型自动显隐，也可在偏好设置里手动固定显示或隐藏。
+ [调整] 前端改为本地模式与服务器模式并存，默认仍是本地模式，可在「配置与用户偏好 - 服务器」中开启。
